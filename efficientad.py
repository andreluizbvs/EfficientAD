...
def get_argparse():
    ...
    parser.add_argument(
        "-t", "--train_steps", type=int, default=(70000 // batch_sz)
    )
...
def main():
    ...
    train_output_dir = os.path.join(
        out_dir, "trainings", config.dataset, config.subdataset
    )
    test_output_dir = os.path.join(
        out_dir, "anomaly_maps", config.dataset, config.subdataset, "test"
    )
    ...
    for iteration, (image_st, image_ae), image_penalty in zip(
        ...
        if iteration % 1000 // batch_sz == 0:
            torch.save(
                teacher, os.path.join(train_output_dir, "teacher_tmp.pth")
            )
            torch.save(
                student, os.path.join(train_output_dir, "student_tmp.pth")
            )
            torch.save(
                autoencoder,
                os.path.join(train_output_dir, "autoencoder_tmp.pth"),
            )
    ...
    torch.save(
        autoencoder, os.path.join(train_output_dir, "autoencoder_final.pth")
    )
    ...
    torch.save(
        {"teacher_mean": teacher_mean, 
         ...
         "q_ae_start": q_ae_start,
         "q_ae_end": q_ae_end},
        f"{config.subdataset}_mean_std.pth",
    )
    auc = test(
        test_set=test_set,
        teacher=teacher,
